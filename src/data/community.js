...
export const POST_TYPES = ['Photo', 'Video', 'Memory'];
...
const seedPosts = [
  {
    ...
    authorId: 'AM97001',
    type: 'Photo',
    caption: 'Memories from the old assembly hall.',
    ...
    comments: [
      createComment({
        ...
        authorId: 'AM97021',
        text: 'These photos bring back great energy. Please share more.',
        ...
        replies: [
          createComment({
            ...
            authorId: 'ADMIN001',
            authorRole: 'admin',
            text: 'Thanks for sharing. Approved and visible to all Seniors now.',
  ...
  },
  {
    ...
    authorId: 'AM97033',
    type: 'Photo',
    caption: 'A new submission from the recent gathering — pending approval.',
    mediaUrl: '',
    status: 'pending',
    likedBy: [],
    comments: [],
...
function removeCommentTree(comments, targetId) {
  return comments
    .filter((comment) => comment.id !== targetId)
    .map((comment) => ({ ...comment, replies: removeCommentTree(comment.replies || [], targetId) }));
}
...
export function deleteComment(postId, commentId) {
  const posts = readPosts().map((post) => {
    if (post.id !== postId) return post;
    return { ...post, comments: removeCommentTree(post.comments || [], commentId) };
  });
  writePosts(posts);
  return posts;
}
